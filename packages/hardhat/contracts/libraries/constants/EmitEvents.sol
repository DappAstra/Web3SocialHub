...
contract EmitEvents{
    ...
    //using Types for *;
/*
    function emitBaseInitialized
        (string memory name, string memory symbol, uint256 timestamp) public {
        emit BaseInitialized(name, symbol, timestamp);
    }

    function emitStateSet(
        address caller,
        Types.ProtocolState prevState,
        Types.ProtocolState newState,
        uint256 timestamp
    ) public {
        emit StateSet(
            caller,
